...
//			food_storage <- food_storage + demand;
	...
	action enter_queue{								// Function arranging entering queue 
		...
		if facility_of_choice.queue_open = true{	// if the facility's queue is open 
			do perceive_queue(length(facility_of_choice.queue),facility_of_choice.parallel_served);
	...
	action perceive_queue(int length_of_queue, int parallel_served) {
...
//		For every befriended household in the social network, with a p_of_interaction the emotional state is updated
		...
		loop times: avg_interactions { //friend over: social_network {	
